...
class DeleteCommand(BaseCommand):
    ...
    def configure_parser(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument("--yes", "-y", action="store_true", help="跳过确认（我懂我懂）～")
    ...
    def _delete_work(self, args: argparse.Namespace) -> int:
        if not args.target:
            return self.output.result(False, error="告诉我要删除的作品 ID 或名称哦～")
        ...
        if not self.output.json_mode and not (args.yes or self.output.no_confirm):
            ...
            if not self.output.confirm("删掉就找不回来啦，确定要删除吗？"):
    ...
    def _delete_author(self, args: argparse.Namespace) -> int:
        ...
        if not self.output.json_mode and not (args.yes or self.output.no_confirm):
            self.output.info(f"将删除作者: [cyan]{name}[/cyan] 及其全部作品")
            if not self.output.confirm("删掉就找不回来啦，确定要删除吗？"):
                self.output.info("[dim]已取消，安心～[/dim]")
                return 0
    ...
    def _delete_all(self, args: argparse.Namespace) -> int:
        ...
        if self.output.json_mode:
            return self.output.result(True, data=result)
        self.output.info(f"已清空库: 删除 {result['deleted']} 部作品")
        return 0
