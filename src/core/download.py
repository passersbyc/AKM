...
from src.core.site import infer_site, SITES


def _dq_db(url: str):
    """URL 对应站点的库连接（download_queue 按站点分库）。"""
    return get_site_db(infer_site(url))


def get_pending_urls() -> list[dict]:
    """返回待下载的 URL（有效、未入库、未拉黑），跨站点聚合。"""
    result = []
    for site in SITES:
        db = get_site_db(site)
        rows = db.execute(
        ...
        result.extend({"url": r[0], "author_id": r[1] or "",
...
def append_or_update(entries: list[dict]) -> int:
    ...
    for e in entries:
        ...
        if not url:
            continue
        groups.setdefault(infer_site(url), []).append(e)

    for site, es in groups.items():
        db = get_site_db(site)
        with db:
            for e in es:
                url = e.get("url", "").strip()
