...
class BiqugeDownloader(BaseDownloader):
    ...
    def process_url(self, urls: Union[str, List[str]], mode: str = "both") -> Dict[str, int]:
        ...
        for url in urls:
            ...
            if not book_id:
                logger.error("(・_・;) 无法解析书籍 ID: %s", url)
                stats["failed"] += 1
            ...
            try:
                ...
                logger.error("(T_T) 下载失败: %s - %s", url, e)
                stats["failed"] += 1
    ...
    def _download_book(self, book_id: int, source_url: str) -> str:
        ...
        if result[1] == "ok":
            logger.info("(^_^) 导入成功: %s (%d章)", title, total)
            return "ok"
        else:
            logger.error("(T_T) 导入失败: %s - %s", title, result[1])
            try:
    ...
    def _fetch_chapter(self, book_id: int, chapter_id: int) -> str:
        ...
        try:
            ...
            logger.warning("  (T_T) 第%d章失败: %s", chapter_id, e)
            return ""
