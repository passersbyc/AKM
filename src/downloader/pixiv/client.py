...
class PixivClient:
            ...
            for attempt in range(3):
                try:
                    ...
                    if r.status_code == 429:
                        ...
                        continue
                    if not r.ok:
                        logger.error("(｡•́︿•̀｡) OAuth 登录失败: HTTP %d - %s", r.status_code, r.text[:200])
                        return False
                    result = r.json()
                    self._access_token = result.get("access_token")
                    if not self._access_token:
                        logger.error("(｡•́︿•̀｡) OAuth 响应中无 access_token")
                        return False
                    new_refresh = result.get("refresh_token")
                    if new_refresh and new_refresh != self._refresh_token:
                        self._refresh_token = new_refresh
                    logger.debug("(◕‿◕) Pixiv OAuth 登录成功")
                    return True
                except Exception as e:
                    logger.error("(｡•́︿•̀｡) OAuth 登录异常 (尝试 %d/3): %s", attempt + 1, e)
                    if attempt < 2:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if r.status_code == 404:
                    logger.warning("(・ω・) 404 Not Found: %s", url)
                    return None
                ...
                if attempt < max_retries - 1:
                    self._stop_event.wait(2 * (attempt + 1))
                    continue
                logger.warning("(｡•́︿•̀｡) API 请求最终失败 (%d/%d): %s 状态码=%d 异常=%s",
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                try:
                    if future.result():
                        ...
                        logger.warning("(｡•́︿•̀｡) 下载失败: %s", url)
                except Exception as e:
                    logger.warning("(｡•́︿•̀｡) 下载异常: %s %s", url, e)
            return success
    ...
    def get_text(self, url: str, timeout: int = 15) -> Optional[str]:
        try:
            ...
            logger.debug("(｡•́︿•̀｡) get_text 失败: %s - %s", url, e)
            return None
        ...
        if response.status_code == 429:
            if self._switch_cookie():
                ...
                if self._429_count == 1:
                    logger.warning("(・ω・) HTTP 429 限流，已切换 Cookie，不等待")
                else:
                    logger.debug("(・ω・) HTTP 429，再次切换 Cookie")
                return True
            ...
            if was_set:
                ...
                return True
            self._429_pause.set()
            logger.warning("(｡•́︿•̀｡) HTTP 429 限流，Cookie 池已耗尽，全局暂停 %.0fs", wait)
            self._stop_event.wait(wait)
